  ...
  if (confirmingRevoke) {
    return (
      <Modal title="确认撤销分享" onClose={() => setConfirmingRevoke(false)} data-testid="share-revoke-confirm">
        <div className="p-4">
        <p className="text-[length:var(--type-body)] text-[var(--color-text-secondary)]">撤销后，现有链接将立即失效。此操作无法撤回。</p>
    ...
    <Modal title="分享文档" onClose={onClose} width="max-w-sm" data-testid="share-modal">
      ...
      {!link ? (
        <>
          <fieldset disabled={operation !== null}>
            <legend className="text-[length:var(--type-caption)] text-[var(--color-text-secondary)] mb-2">链接有效期</legend>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {TTL_OPTIONS.map((option) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-[length:var(--type-caption)] border transition-colors ${
                    ttl === option.value
          ...
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-1.5 py-2 bg-[var(--color-accent)] text-[var(--color-text-on-color)] text-[length:var(--type-body)] font-semibold disabled:opacity-50 hover:bg-[var(--color-accent-hover)] transition-colors"
          >
        ...
        <>
          <label htmlFor="share-link" className="block text-[length:var(--type-caption)] text-[var(--color-text-secondary)] mb-1">分享链接</label>
          <div className="flex items-center gap-2 mb-4">
            <input
              ...
              className="flex-1 min-w-0 text-[length:var(--type-body)] bg-[var(--color-bg)] px-3 py-2 border border-[var(--color-border)] text-[var(--color-text-primary)] overflow-hidden text-ellipsis"
            />
            <button
              type="button"
              onClick={handleCopy}
              ...
              className="inline-flex shrink-0 items-center gap-1.5 px-3 py-2 text-[length:var(--type-body)] font-semibold bg-[var(--color-accent)] text-[var(--color-text-on-color)] hover:bg-[var(--color-accent-hover)] disabled:opacity-50"
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={handleCreate}
              disabled={operation !== null}
              className="inline-flex flex-1 items-center justify-center gap-1.5 py-2 border border-[var(--color-border)] text-[var(--color-text-primary)] text-[length:var(--type-caption)] font-semibold hover:bg-[var(--color-bg)] disabled:opacity-50"
            >
            ...
            <button
              ...
              className="inline-flex flex-1 items-center justify-center gap-1.5 py-2 border border-[var(--color-danger)] text-[var(--color-danger-text)] text-[length:var(--type-caption)] font-semibold hover:bg-[color-mix(in_srgb,var(--color-danger)_10%,var(--color-surface))] disabled:opacity-50"
            >
      ...
      {feedback && (
        <p
          ...
          className={`mt-3 text-[length:var(--type-caption)] ${feedback.kind === 'error' ? 'text-[var(--color-danger-text)]' : 'text-[var(--color-success-text)]'}`}
        >
