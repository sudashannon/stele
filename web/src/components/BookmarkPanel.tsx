  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto">
        {bookmarks.length === 0 ? (
          <div className="px-4 py-8 text-[var(--type-caption)] text-[var(--color-text-secondary)]">
            尚无收藏。可在文档工具栏中点击“收藏”保存当前文档。
          </div>
        ) : (
          <ul className="divide-y divide-[var(--color-border)]">
            {bookmarks.map((bookmark) => (
              <li key={bookmark.path} className="flex items-center hover:bg-[var(--color-layer)]">
                ...
                >
                  <span className="shrink-0 border border-[var(--color-accent)] bg-[var(--color-accent-subtle)] px-1.5 py-0.5 text-[var(--type-caption)] font-medium uppercase text-[var(--color-accent)]">
                    {bookmark.type}
                  </span>
                  <span className="truncate text-[var(--type-caption)] text-[var(--color-text-primary)]" title={bookmark.title}>
                    {bookmark.title}
                  </span>
                </button>
                <button
                  ...
                  className="shrink-0 self-stretch px-4 text-[var(--color-text-tertiary)] hover:text-[var(--color-danger)]"
                >
                  <Icon name="trash" />
                </button>
              </li>
